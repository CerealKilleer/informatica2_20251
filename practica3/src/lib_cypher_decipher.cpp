#include <iostream>
#include <lib_cypher_decipher.hpp>


namespace {
    int compare_zeros_ones(uint8_t *bits, uint32_t n)
    {
        /***
         * Retorna 3 si hay mas 1s, 2 si hay mas ceros y 1 si hay igual cantidad
         *  ***/

        uint32_t zeros_cnt = 0;
        uint32_t ones_cnt = 0;
        for (int i=0; i < n; i++) {
            if (!(bits[i]))
                zeros_cnt++;
        }

        ones_cnt = n - zeros_cnt;
        if (ones_cnt > zeros_cnt)
            return EVERY_THREE;
        else if (ones_cnt < zeros_cnt)
            return EVERY_TWO;
        else
            return EVERY_ONE;
    }

    void change_bits(uint32_t n, uint32_t m, uint8_t row, uint8_t col, uint8_t **matrix, uint32_t rows, uint32_t cols)
    {
        uint8_t bits_cnt = 0;
        uint8_t j=col;
        for (uint8_t i=row; i < rows; i++) {
            while (j < cols) {
                bits_cnt++;
                if (!(bits_cnt % m)) {
                    //std::cout << "Posicion: i= " << (int)i << " j= " << (int)j << " Valor: " << (int)matrix[i][j] << " Cambio: " << !matrix[i][j] << std::endl;
                    matrix[i][j] = !matrix[i][j];
                }
                if (bits_cnt == n)
                    return;
                j++;
            }
            j=0;
        }
    }

    bool get_groups(uint32_t n, uint8_t *group, uint8_t row, uint8_t col, uint8_t **matrix, uint32_t rows, uint32_t cols) 
    {
    uint32_t k = 0;
    uint8_t j=col;
    for (uint8_t i=row; i<rows; i++) {
        while (j<cols) {
            group[k++] = matrix[i][j];
            if (!(k % n))
                return true;
            j++;
        }
        j=0;
    }
    return false;
    }
}


bool cypher_1(uint8_t **matrix, uint32_t n, uint32_t rows, uint32_t cols)
{
    uint8_t group[n];
    uint8_t m, m_aux;
    uint32_t k = 0;
    if (!get_groups(n,group,0,0,matrix,rows,cols)) {
        std::cout << "No hay suficientes datos" << std::endl;
        return false;
    }
    
    m = compare_zeros_ones(group, n);
    change_bits(n,EVERY_ONE,0,0,matrix,rows,cols);

    for (int i=0; i < rows; i++) {
        for (int j=0; j < cols; j++) {
            if (k == n) {
                get_groups(n,group,i,j,matrix,rows,cols);
                m_aux =  compare_zeros_ones(group, n);
                change_bits(n,m,i,j,matrix,rows,cols);
                m = m_aux;
                k = 0;
            }
            k++;
        }
    }
    return true;
}